...
import { tokenize as tokenizer8 } from '@projectwallace/css-parser';
import { projectwallaceToUniversal } from '../util/projectwallace-to-universal.mjs';
...
export function tokenize(tokenizerName, source, config) {
	...
	if (tokenizerName === '@cdoublev/css') {
		...
		try {
			...
			// console.log('@cdoublev/css', err, source);
		}

		return result;
	}

	if (tokenizerName === "@projectwallace/css-parser") {
		const result = [];

		for (const token of tokenizer8(source)) {
			const uToken = projectwallaceToUniversal(token, source);
			if (!config.includeComments && uToken.type === "comment") {
				continue;
			}
