...
async function store(tokens) {
	...
	await fs.writeFile(`./tests/fuzz/${id}/tokens.json`, JSON.stringify(tokens, null, '\t'), 'utf-8');
	await fs.writeFile(`./tests/fuzz/${id}/source.css`, resultStr, 'utf-8');
}
...
export async function testFuzz() {
	...
	const expect = tokenize('csslex', source, { includeComments: true });
	...
	for (let i = 0; i < result.length; i++) {
		...
		if (!expectToken || !actualToken) {
			...
			console.log(result.slice(Math.max(0, i - 2), i + 2));
			console.log(expect.slice(Math.max(0, i - 2), i + 2));
			console.log('-------------------------------');

			await store(result.slice(Math.max(0, i - 5), i + 5));
			break;
		}
